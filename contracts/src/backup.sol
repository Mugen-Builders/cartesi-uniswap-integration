...
contract USDCVaultWithLiquidity is SafeCallback {
    ...
    uint256 private constant ADD_LIQUIDITY = 1;
    uint256 private constant REMOVE_LIQUIDITY = 2;
    ...
    function withdrawToUser(address user, uint256 usdcAmount) external {
        ...
        if (idle >= usdcAmount) {
            ...
            if (needFromLp > totalDeployed) revert InsufficientDeployed();
            bytes memory data = abi.encode(REMOVE_LIQUIDITY, needFromLp);
            poolManager.unlock(data);
            totalDeployedInLp -= needFromLp;
