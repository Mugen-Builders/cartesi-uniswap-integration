...
Interacts with the merged vault-with-liquidity contract on the base layer:
  - addLiquidityFromApplication(uint256): vault pulls USDC from the dApp and adds to Uniswap v4 LP.
...
VAULT_ADDRESS = environ.get("VAULT_ADDRESS", "0xeb7069D1cB29D59079C2558219C32Bb4F5c58a75")
...
VAULT_APPROVAL_AMOUNT = int(environ.get("VAULT_APPROVAL_AMOUNT", "1000000000000"))  # 1_000_000 * 1e6
# ERC20 Portal address (chain-specific; use cartesi address-book or deploy config)
ERC20_PORTAL_ADDRESS = environ.get("ERC20_PORTAL_ADDRESS", "0x9C21AEb2093C32DDbC53eEF24B873BDCd1aDa1DB").lower()
...
def decode_erc20_deposit(payload_hex: str) -> tuple[bool, str, str, int, bytes]:
    ...
    Cartesi ERC20 Portal uses a packed layout (not 32-byte ABI aligned):
      - 1 byte: success (bool, 0x00 or 0x01)
      - 20 bytes: token address
      - 20 bytes: sender address
      - 32 bytes: amount (uint256 big-endian)
      - optional: execLayerData (remaining bytes)
    """
    data = hex2bytes(payload_hex)
    if len(data) < 73:
        raise ValueError(f"payload too short for ERC20 deposit: got {len(data)} bytes, need at least 73")

    success = data[0] != 0
    token = "0x" + data[1:21].hex()
    sender = "0x" + data[21:41].hex()
    amount = int.from_bytes(data[41:73], "big")
    exec_layer_data = data[73:] if len(data) > 73 else b""
    return success, token, sender, amount, exec_layer_data
...
SELECTORS = {
    "addLiquidityFromApplication(uint256)": "2c1b1e51",
...
def _encode_add_liquidity_from_application(amount: int) -> str:
    return "0x" + SELECTORS["addLiquidityFromApplication(uint256)"] + _abi_encode_uint256(amount)
...
def handle_advance(data: dict) -> str:
    ...
    # DApp Address Relayer: store the application (dApp) contract address from the relay input.
    # See https://docs.cartesi.io/cartesi-rollups/1.5/rollups-apis/json-rpc/relays
    if DAPP_ADDRESS_RELAY_ADDRESS and msg_sender == DAPP_ADDRESS_RELAY_ADDRESS:
        try:
            app_addr = _decode_application_address_from_payload(payload_hex)
            if app_addr and _is_valid_eth_address(app_addr):
                APPLICATION_ADDRESS = app_addr.strip().lower()
                emit_notice({"action": "application_address", "application_address": APPLICATION_ADDRESS})
                logger.info("Application address set from relayer: %s", APPLICATION_ADDRESS)
                return "accept"
            emit_report("DApp address relayer: invalid payload (expected 20 or 32 bytes)")
        except Exception as e:
            emit_report(f"DApp address relayer decode failed: {e}")
        return "reject"
    ...
    if ERC20_PORTAL_ADDRESS and msg_sender == ERC20_PORTAL_ADDRESS:
        ...
        while (total_deposited - total_deployed_lp) >= DEPLOY_THRESHOLD:
            try:
                add_voucher(VAULT_ADDRESS, _encode_add_liquidity_from_application(DEPLOY_THRESHOLD))
                total_deployed_lp += DEPLOY_THRESHOLD
                emit_notice({"action": "add_liquidity", "amount": DEPLOY_THRESHOLD})
            except Exception as e:
